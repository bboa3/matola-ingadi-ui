...
import { events, findEventById } from '@utils/events'
...
const Home: React.FC = () => {
  ...
  const [selectedColor, setSelectedColor] = useState(event.colors[0])
  const [selectedSize, setSelectedSize] = useState(event.sizes[2])
              ...
              {events.map((event, index) => {
                ...
                return (
                  <li key={event.id} className="text-sm">
                    <a href={event.href} aria-current="page" className="font-medium text-gray-500 hover:text-gray-700">
                      {event.name}6
                    </a>
                  </li>
                )
