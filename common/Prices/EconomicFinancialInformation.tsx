...
const EconomicFinancialInformation: React.FC<Props> = ({ pricing, locale }) => {
  const informationPricing = activityFilter({ pricing, activityId: 'economic-and-financial-information' })
  const lang = getLanguage(locale!)

  return (
    <section className='w-full text-white'>
      <div className='w-full h-full flex flex-col items-center justify-center md:px-5 pb-6 pt-14 lg:pb-12'>
          ...
          <div className='w-full max-w-5xl h-4/5'>
            <div className='w-full'>
              <div className='md:text-center pt-9 md:pt-12 flex flex-col justify-center items-center'>
                <h1 className='font-bold text-4xl md:text-5xl text-white'>{lang.informationPricing.title.h1}</h1>
                <p className='text-gray-300 pt-3'>{lang.informationPricing.title.p2}</p>
              </div>
            </div>
            <div className='w-full md:h-96 relative -bottom-9 md:-bottom-12  md:grid md:grid-cols-3 space-y-5 md:space-y-0 gap-4'>
            {
              informationPricing.map(price => (
...
export default EconomicFinancialInformation
