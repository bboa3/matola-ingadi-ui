...
const CardNextEvents: React.FC<Props> = ({ invoices }) => {
  const getNextEvents = useCallback(() => {
    ...
    for (const invoice of invoices) {
      const { eventDate, invoiceStatus } = invoice

      if (now.isBefore(eventDate) && invoiceStatus === 'PAID') {
            ...
            </thead>
            {
              nextEvents[0]
                ? (
                  <tbody>
                    {
                      invoices.map(({ invoiceCode, services, eventDate, eventType, guestsNumber }) => {
                        const eventDateFormatted = createDate(eventDate).format('DD/MM/YYYY')
